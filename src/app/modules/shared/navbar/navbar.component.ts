...
export class NavbarComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        /*this.client.getOptions().subscribe((menu: any) => {
            this.listTitles = menu.data.map((item) => {
                return {
                    path: item.name,
                    title: item.text,
                    type: item.menu ? 'sub' : 'link',
                    icontype: 'dashboard',
                    collapse: item.menu ? item.name : undefined,
                    children: item.nodes ? item.nodes.map((node) => {
                        return {
                            path: node.name,
                            title: node.text,
                            ab: node.text.substring(0, 2)};
                    }) : null
                };
            });
            this.spinner.hide();
    ...
    getTitle() {
        if (this.listTitles) {
            const titlee: any = this.location.prepareExternalUrl(this.location.path()).replace('#/','');
